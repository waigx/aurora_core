...
class Discover(object):
    ...
    def __discover_and_auth(self):
        self.zeroconf = Zeroconf()
        self.handler = DiscoverHandler(self.__discovered_callback)
        ...
        while not self.current_authed:
            ...
            for aurora, info in self.aurora_info_dict.items():
                ...
                if new_auth_req.status_code == 200:
                    ...
                    self.__authed_callback(info, new_auth_req.json())
                    break;

        self.zeroconf.close()

    def __discovered_callback(self, aurora_info):
    ...
    def __authed_callback(self, aurora_info, auth_data):
